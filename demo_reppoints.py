...
if __name__ == '__main__':
    ...
    _decode = Decode_RepPoints(model, class_names, place, cfg, for_test=True)
    ...
    image = dic['image']
    pimage = dic['pimage']
    img_metas = dic['img_metas']


    # warm up
    if use_gpu:
        for k in range(10):
            image, boxes, scores, classes = _decode.detect_image(image, pimage, img_metas, draw_image=False)
    ...
    for k, filename in enumerate(path_dir):
        ...
        image, boxes, scores, classes = _decode.detect_image(image, pimage, img_metas, draw_image, draw_thresh)
