...
model_path = 'solov2_light_r50_vd_fpn_dcn_512_3x.pdparams'
...
for lvl in range(0, num_convs):
    # conv + gn
    if lvl in dcn_v2_stages:  # DCNv2
        offset_w = state_dict['bbox_head.kernel_convs.%d_conv_offset.w_0'%lvl]
        offset_b = state_dict['bbox_head.kernel_convs.%d_conv_offset.b_0'%lvl]
        copy_conv('head.krn_convs.%d.conv' % (lvl, ), offset_w, offset_b)

        w = state_dict['bbox_head.kernel_convs.%d_weights'%lvl]
        scale = state_dict['bbox_head.kernel_convs.%d.gn_scale'%lvl]
        offset = state_dict['bbox_head.kernel_convs.%d.gn_offset'%lvl]
        conv_unit_name = 'head.krn_convs.%d' % (lvl, )
        copy(conv_unit_name + '.dcn_param', w)
        copy(conv_unit_name + '.gn.weight', scale)
        copy(conv_unit_name + '.gn.bias', offset)
    else:
    ...
    # conv + gn
    if lvl in dcn_v2_stages:  # DCNv2
        offset_w = state_dict['bbox_head.cate_convs.%d_conv_offset.w_0'%lvl]
        offset_b = state_dict['bbox_head.cate_convs.%d_conv_offset.b_0'%lvl]
        copy_conv('head.cls_convs.%d.conv' % (lvl, ), offset_w, offset_b)

        w = state_dict['bbox_head.cate_convs.%d_weights'%lvl]
        scale = state_dict['bbox_head.cate_convs.%d.gn_scale'%lvl]
        offset = state_dict['bbox_head.cate_convs.%d.gn_offset'%lvl]
        conv_unit_name = 'head.cls_convs.%d' % (lvl, )
        copy(conv_unit_name + '.dcn_param', w)
        copy(conv_unit_name + '.gn.weight', scale)
        copy(conv_unit_name + '.gn.bias', offset)
    else:
...
copy_conv('head.cls_convs.%d.conv' % (num_convs+1,), w, b)

# 卷积核分支最后的conv
w = head_dic['bbox_head.solo_kernel.weight']
b = head_dic['bbox_head.solo_kernel.bias']
copy_conv('head.krn_convs.%d.conv' % (num_convs+1,), w, b)
...
save_name = 'dygraph_solov2_light_r50_vd_fpn_dcn_512_3x.pdparams'
