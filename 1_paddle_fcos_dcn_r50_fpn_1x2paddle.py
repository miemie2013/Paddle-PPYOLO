...
model_path = 'fcos_dcn_r50_fpn_1x.pdparams'
...
for nid, num in enumerate(nums):
    ...
    for kk in range(num):
        ...
        if (nid+2) in cfg.backbone['dcn_v2_stages']:   # DCNv2
...
for lvl in range(0, num_convs):
    ...
    if lvl in cfg.head['dcn_v2_stages']:  # DCNv2
        offset_w = state_dict[conv_cls_name + '_conv_offset.w_0']
        offset_b = state_dict[conv_cls_name + '_conv_offset.b_0']
        conv_name = 'head.cls_convs.%d' % (lvl, )
        copy_conv(conv_name+'.conv', offset_w, offset_b)

        w = state_dict[conv_cls_name + '_weights']
        b = state_dict[conv_cls_name + '_bias']
        scale = state_dict[norm_name + '_scale']
        offset = state_dict[norm_name + '_offset']
        copy(conv_name + '.dcn_param', w)
        copy(conv_name + '.dcn_bias', b)
        copy(conv_name + '.gn.weight', scale)
        copy(conv_name + '.gn.bias', offset)
    else:
    ...
    if lvl in cfg.head['dcn_v2_stages']:  # DCNv2
        offset_w = state_dict[conv_reg_name + '_conv_offset.w_0']
        offset_b = state_dict[conv_reg_name + '_conv_offset.b_0']
        conv_name = 'head.reg_convs.%d' % (lvl, )
        copy_conv(conv_name+'.conv', offset_w, offset_b)

        w = state_dict[conv_reg_name + '_weights']
        b = state_dict[conv_reg_name + '_bias']
        scale = state_dict[norm_name + '_scale']
        offset = state_dict[norm_name + '_offset']
        copy(conv_name + '.dcn_param', w)
        copy(conv_name + '.dcn_bias', b)
...
save_name = 'dygraph_fcos_dcn_r50_fpn_1x.pdparams'
